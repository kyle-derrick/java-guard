...
- **Compilable Class Stubs**: Preserves API metadata while replacing implementations with default bodies; unnamed `MethodParameters` remain unnamed and are not copied into LocalVariableTable entries, whose names cannot be null.
...
| `-l, --launcher` | Enable native launcher compilation and Java runtime packaging; may be used without an input JAR for launcher-only packaging |
...
## ✅ Compatibility Automation and Validation Scope
...
See [compat-tests/README.md](compat-tests/README.md) for fixture details, supported combinations, and local runner commands. See [GitHub Actions](https://github.com/kyle-derrick/java-guard/actions) for current CI status.
